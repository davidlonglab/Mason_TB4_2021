...
## Daniyal Jafree & Gideon Pomeranz | 30th December 2020 | Version 4 | Acquisition of post-aligned files from GEO accession (https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE146912) and comparison of TB4 expression with statistics.
...
setwd("/Users/daniyaljafree/Desktop/glom_analysis/Will/New_analysis")
...
ctrl2.data <- read.table(file=paste0("/Users/daniyaljafree/Desktop/glom_analysis/Will/New_analysis/GSE146912_RAW/GSM4409508_control_2.txt.gz"),sep="\t", header = T, row.names = NULL)
...
# Add 'project = "Ctrl"' to differentiate between control and ADR samples.
ctrl3.data <- read.table(file=paste0("/Users/daniyaljafree/Desktop/glom_analysis/Will/New_analysis/GSE146912_RAW/GSM4409509_control_3.txt.gz"),sep="\t", header = T, row.names = NULL)
...
adr1.data <- read.table(file=paste0("/Users/daniyaljafree/Desktop/glom_analysis/Will/New_analysis/GSE146912_RAW/GSM4409514_doxorubicin_1.txt.gz"),sep="\t", header = T, row.names = NULL)
...
# Add 'project = "Adr"' to differentiate between control and ADR samples.
adr2.data <- read.table(file=paste0("/Users/daniyaljafree/Desktop/glom_analysis/Will/New_analysis/GSE146912_RAW/GSM4409515_doxorubicin_2.txt.gz"),sep="\t", header = T, row.names = NULL)
...
new.cluster.ids <- c("GEC", "GEC", "Mesangium", "Podocyte", "AEC", "Macrophage", "PEC", "SMC", "Monocyte", "TEC", "PEC", "Mesangium", "T lymphocyte", "Podocyte")
names(new.cluster.ids) <- levels(Combined)
Combined <- RenameIdents(Combined, new.cluster.ids)

# Plot UMAP assigned by cell type (Figure 1A).
DimPlot(Combined, reduction = "umap", label = T, pt.size = 1, label.size = 0)
...
cell_numbers <- table(Combined@active.ident, Combined@meta.data$orig.ident)
cell_numbers
write.csv(cell_numbers, "cell_numbers.csv")


## PART 5: Differential expression between Ctrl and Adr for Tmsb4x.
...
# Compare TB4 expression in grouped cells between conditions and save as CSV in current directory. Includes data used for Wilcoxon Rank Sum test used in Figure 1C.
